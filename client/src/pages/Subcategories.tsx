...
const Subcategories = () => {
  ...
  const [formData, setFormData] = useState({ name: '', category: '', image: '', status: true });
  ...
  const handleAdd = () => {
    ...
    setFormData({ name: '', category: '', image: '', status: true });
    setImagePreview('');
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingSubcategory) {
        await subcategoryAPI.update(editingSubcategory._id, submitData);
      } else {
  ...
  const columns = [
    ...
    { key: 'name', header: 'SUB CATEGORY NAME' },
    {
      ...
      render: (item: Subcategory) => getCategoryName(item.category),
    },
    {
      key: 'image',
      header: 'IMAGE',
  ...
  const confirmDelete = async () => {
    ...
    try {
      await subcategoryAPI.delete(deleteConfirm._id);
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="bg-purple-600 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4 flex-1">
          <h1 className="text-xl font-bold text-white">Sub Category</h1>
      ...
      <Modal
        ...
        title={`${editingSubcategory ? 'Edit' : 'Add'} Sub Category`}
        showBackButton={true}
      >
